...
prop_freshName' = unNom $ do -- Nom monad 
   a <- (freshName' :: Nom (Name Int)) 
   b <- freshName' 
   return $ a /= b && nameLabel a == nameLabel b && nameLabel a == def
--}
...
prop_transposeNomMaybe x' = unNom $ do -- Nom monad
    ...
    l2' <- transposeMaybeNom (transposeNomMaybe x') -- Maybe get another list of names (freshening means the atoms will be different) 
...
-- prop_transposeMaybeNom Nothing   = isNothing $ transposeNomMaybe (transposeMaybeNom Nothing)
prop_transposeMaybeNom x' = 
    let y' = transposeNomMaybe (transposeMaybeNom x') 
    in  case (x', y') of
